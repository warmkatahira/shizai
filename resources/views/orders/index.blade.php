    ...
    <div class="flex items-center justify-between mb-6">
        ...
        @if (auth()->user()->isSales())
            <a href="{{ route('orders.create') }}"
               class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-4 py-2 rounded-md">＋ 新規発注申請</a>
        @endif
    </div>
    ...
    <form method="GET" action="{{ route('orders.index') }}" class="bg-white shadow rounded-lg p-4 mb-6">
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <div>
                ...
                <select name="status" class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-indigo-500">
            ...
            @if ($offices->isNotEmpty())
                <div>
                    ...
                    <select name="office_id" class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-indigo-500">
                        <option value="">すべて</option>
            ...
            <div>
                ...
                <select name="supplier_id" class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-indigo-500">
                    <option value="">すべて</option>
            ...
            <div>
                ...
                <input type="text" name="keyword" value="{{ $filters['keyword'] ?? '' }}" placeholder="例：用紙"
                       class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-indigo-500">
            </div>
        ...
        <div class="flex items-center gap-3 mt-4">
            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-5 py-2 rounded-md">検索</button>
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                @empty
                    <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">発注申請がまだありません。</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
