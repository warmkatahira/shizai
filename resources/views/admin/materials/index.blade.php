...
{{-- 閲覧だけの人には「管理」と言わない --}}
@section('title', auth()->user()->canManageMasters() ? '資材マスタ管理' : '資材マスタ')
...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-xl font-bold">{{ $canEditMasters ? '資材マスタ管理' : '資材マスタ' }}</h1>
        @if ($canEditMasters)
    ...
    <div class="bg-white shadow rounded-lg overflow-auto max-h-[70vh]">
        <table class="w-full text-sm whitespace-nowrap">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                @empty
                    <tr><td colspan="{{ $canEditMasters ? 15 : 14 }}" class="px-4 py-8 text-center text-gray-400">資材がまだありません。</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
