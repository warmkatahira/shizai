...
{{-- 閲覧だけの人には「管理」と言わない --}}
@section('title', auth()->user()->canEditMaster('materials') ? '資材マスタ管理' : '資材マスタ')

@section('content')
    @php
        // このマスタを編集できるか（管理者・総務は常に可。それ以外はユーザー管理の「編集」トグル）。
        // オフの人は一覧を読むだけなので、登録・編集・削除・CSVは丸ごと出さない
        $canEditMaster = auth()->user()->canEditMaster('materials');
    @endphp
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-xl font-bold">{{ $canEditMaster ? '資材マスタ管理' : '資材マスタ' }}</h1>
        @if ($canEditMaster)
    ...
    <div class="bg-white shadow rounded-lg overflow-auto max-h-[70vh]">
        <table class="w-full text-sm whitespace-nowrap">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                @empty
                    <tr><td colspan="{{ $canEditMaster ? 15 : 14 }}" class="px-4 py-8 text-center text-gray-400">資材がまだありません。</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
