...
@section('content')
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {{-- 営業所ユーザー向け --}}
        @if ($user->isSales())
            <a href="{{ route('orders.create') }}" class="block bg-white rounded-lg shadow p-5 hover:ring-2 hover:ring-indigo-200">
                <h2 class="font-semibold mb-1">資材を発注する</h2>
                <p class="text-sm text-gray-500">必要な資材を選んで発注申請します。</p>
            </a>
            <a href="{{ route('orders.index') }}" class="block bg-white rounded-lg shadow p-5 hover:ring-2 hover:ring-indigo-200">
        ...
        {{-- 管理者向け --}}
        @if ($user->isAdmin())
            <a href="{{ route('admin.offices.index') }}" class="block bg-white rounded-lg shadow p-5 hover:ring-2 hover:ring-indigo-200">
                <h2 class="font-semibold mb-1">営業所管理</h2>
                <p class="text-sm text-gray-500">営業所を登録・編集します。</p>
            </a>
            <a href="{{ route('admin.users.index') }}" class="block bg-white rounded-lg shadow p-5 hover:ring-2 hover:ring-indigo-200">
                <h2 class="font-semibold mb-1">ユーザー管理</h2>
                <p class="text-sm text-gray-500">利用者を登録・編集します。</p>
            </a>
            <a href="{{ route('admin.suppliers.index') }}" class="block bg-white rounded-lg shadow p-5 hover:ring-2 hover:ring-indigo-200">
                <h2 class="font-semibold mb-1">業者マスタ管理</h2>
                <p class="text-sm text-gray-500">資材の仕入先業者を登録・編集します。</p>
            </a>
            <a href="{{ route('admin.materials.index') }}" class="block bg-white rounded-lg shadow p-5 hover:ring-2 hover:ring-indigo-200">
