...
class MaterialCsv extends MasterCsv
{
    ...
    public static function row(Model $material): array
    {
        return [
            ...
            $material->width_mm,   // W（幅長）
            $material->length_mm,  // D（奥行巾）
            $material->height_mm,  // H（高さ）
    ...
    protected static function attributes(array $cols, array $context): array
    {
        return [
            ...
            'width_mm' => self::nullableNumber($cols[4]),   // W（幅長）
            'length_mm' => self::nullableNumber($cols[5]),  // D（奥行巾）
            'height_mm' => self::nullableNumber($cols[6]),  // H（高さ）
            // $cols[7] は3辺計。W・D・Hから求まる計算値なので取り込まない（直しても無視される）
